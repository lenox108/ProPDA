...
class EditPostViewModel @Inject constructor(
    ...
    fun exit() {
        // Персистентный черновик НЕ чистим при выходе/discard: «недописанный ответ ждёт в теме».
        // Черновик темы (ключ topic:<id>) общий с компактной панелью темы и снимается только при
        // успешной отправке (clearPersistedDraft в onSuccess) или ручной очистке поля (save("")).
        router.exit()
    }
