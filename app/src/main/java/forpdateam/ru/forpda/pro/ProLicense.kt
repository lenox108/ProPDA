...
object ProLicense {
     ...
     * Лежит в XOR-виде: сам по себе он не секрет, но в открытом виде это удобный якорь —
     * перекрёстные ссылки от него ведут прямо к проверке подписи.
     */
    private val PUBLIC_KEY_B64: String
        get() = deobfuscate(
    ...
    /**
     * Имена ключей в настройках — тоже в XOR-виде, как и [MESSAGE_PREFIX]. В открытом виде
     * «pro.license_key» — удобный якорь: находится поиском по строкам и ведёт перекрёстными
     * ссылками прямо к проверке. Значения ключей прежние, миграция не нужна.
     */
    val KEY_LICENSE: String
        get() = deobfuscate(0x2a, 0x28, 0x35, 0x74, 0x36, 0x33, 0x39,
                0x3f, 0x34, 0x29, 0x3f, 0x05, 0x31, 0x3f, 0x23)

    private val KEY_MEMBER_ID: String
        get() = deobfuscate(0x37, 0x3f, 0x37, 0x38, 0x3f, 0x28, 0x05, 0x33, 0x3e)
