...
internal fun preferMentionPostUrl(rowHtml: String, primaryHref: String): String {
    ...
    firstMatchingHref { u -> Regex("(?i)findpost|view=findpost").containsMatchIn(u) }
            ?.let { return absolutizeForumHref(it) }
    ...
    if (showtopic != null && postId != null) {
        return "${SiteUrls.BASE_HTTPS}/forum/index.php?showtopic=$showtopic&view=findpost&p=$postId"
    }

    return absolutizeForumHref(normPrimary)
}
    ...
    fun parse(response: String): MentionsData = MentionsData().also { data ->
        patternProvider
                ...
                .findAll { matcher ->
                    ...
                    var link = preferMentionPostUrl(rowHtml, matcher.group(3).orEmpty())
...
internal fun patchMentionLinkIfTopicOnly(link: String, rowHtml: String): String {
    if (Regex("(?i)findpost|view=findpost").containsMatchIn(link)) return link
    ...
    return "${SiteUrls.BASE_HTTPS}/forum/index.php?showtopic=$topicId&view=findpost&p=$postId"
}
