...
class ArticleParser(
    ...
    private fun parseDataSitePoll(encodedPayload: String): DataSitePoll? {
        val payload = encodedPayload.articleFromHtml()?.articleFromHtml()
                ?.replace('\u00A0', ' ')
                ?.trim()
                .orEmpty()
        if (payload.isBlank()) return null
        return parseDataSitePollJson(payload) ?: parseDataSitePollCompat(payload)
    }

    private fun parseDataSitePollJson(payload: String): DataSitePoll? =
            runCatching {
    ...
    private fun parseDataSitePollCompat(payload: String): DataSitePoll? {
        ...
        return mergeDataSitePollVoteState(
                pollId = pollId,
                title = title,
                multiSelect = multiSelect,
                options = options,
    ...
    private fun mergeDataSitePollVoteState(
            pollId: String,
            title: String,
            multiSelect: Boolean,
