...
import forpdateam.ru.forpda.ui.views.dialog.MaterialAlertDialogHelper;
...
public class ThemeDialogsHelper_V2 {
    ...
    public ThemeDialogsHelper_V2(Context context, AuthHolder authHolder, OtherPreferencesHolder otherPreferencesHolder) {
        ...
        postMenu.addItem(App.get().getString(R.string.vote_bad), (p, data) -> votePost(p, data, false));
        postMenu.addItem(App.get().getString(R.string.vote_good), (p, data) -> votePost(p, data, true));
    ...
    public void showPostMenu(ThemeWebCallbacks presenter, IBaseForumPost post) {
        ...
        AuthData authData = authHolder.get();
        // Репутация поста (как бывшие кнопки в подвале) — не для своего сообщения
        if (!authData.isAuth() || post.getUserId() != authData.getUserId()) {
            postMenu.allow(2);
            postMenu.allow(3);
        }
