...
abstract class BaseAdapter<E, VH : BaseViewHolder<E>> : RecyclerView.Adapter<VH>() {
    ...
    /**
     * Stable-identity check for [DiffUtil] («тот же элемент»). Default — сравнение по `equals`.
     * Subclasses whose items are re-created on each load (e.g. search results) should override to compare
     * by a stable id so DiffUtil can reuse rows and animate instead of treating every reload as a full
     * replace (O(n·m) diff for nothing).
     */
    protected open fun areItemsSame(oldItem: E, newItem: E): Boolean = oldItem == newItem

    /** Content-equality check for [DiffUtil] («содержимое не изменилось» → без ре-байнда). */
    protected open fun areContentsSame(oldItem: E, newItem: E): Boolean = oldItem == newItem
