 ...
 *
 * Внешних крэш-репортеров в сборке нет, поэтому это единственное место, где падение
 * фиксируется в доступном пользователю виде.
 *
 * Репортер ставит глобальный [Thread.UncaughtExceptionHandler]:
 ...
 *
 * Цепочка обработчиков сохраняется: ранее установленный системный handler вызывается
 * после записи файла, поэтому штатное поведение (диалог ANR/краша) не теряется.
 */
