    ...
    private func row(_ c: Client) -> some View {
        HStack(spacing: 8) {
            ...
            if editing == c.memberId {
                ...
                // Выделение текста здесь намеренно НЕ включено: в выделяемом тексте двойной
                // клик выделяет слово и до жеста правки дело не доходит. Заметка чаще нужна
                // для правки, чем для копирования, поэтому приоритет у двойного клика.
