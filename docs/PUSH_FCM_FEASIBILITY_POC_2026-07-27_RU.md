...
## Событийный канал :993 — разобран и признан мёртвым (30.07.2026)

Задача: понять, почему realtime-сокет ProPDA подключается, подписку сервер подтверждает, а
события не приходят. Проверено двумя независимыми экспериментами; в обоих доказано, что
сообщения РЕАЛЬНО дошли до аккаунта (по FCM-пушу либо по счётчику инспектора).

**1. Легаси-канал `ea` (то, чем пользуется ProPDA).** Отдельный python-клиент с мака
(`scratchpad/ws993.py`, голый TCP + WS-фреймы). Сервер отвечает и различает команды:

| запрос | ответ | смысл |
|---|---|---|
| `[1,"sv"]` | `[1,0,"0.4.2b"]` | известная команда, версия протокола |
| `[2,"zz"]` | `[2,2]` | **статус 2 = неизвестная команда** |
| `[3,"ea","u<memberId>"]` | `[3,0]` | `ea` известна, статус 0 = принято |
| `[5,"ah"]` | бинарь (RSV1-deflate) | это новый app-протокол, тот же порт |
| `[6,"ev"]`, `[7,"sb",…]` | статус 2 | таких команд нет |

⚠️ Имя канала сервер НЕ валидирует: `[0,"ea","q…"]`, `[0,"ea","<id>"]` и даже `[0,"ea"]` без
аргумента получают тот же `0`. То есть «принято» ничего не доказывает.
Замер: соединение с подпиской `u<memberId>` держалось открытым, собеседник прислал сообщение
(доказательство доставки — три `FCM push received` в 19:43:41/43/53) → на канале только pong'и,
**ни одного события**.

**2. Новый app-протокол авторизованной сессией.** Временный пробник в приложении:
`AppProtocolClient.connectAny()` → `ma` (resume по сохранённому `login_key`) → чтение всех
входящих кадров. `resume=true`, сессия авторизована.
- Сервер рвёт соединение ровно через 60с бездействия (`EOFException`) — лечится ping'ом, после
  чего сокет живёт сколько угодно.
- За время жизни соединения собеседник прислал 2 сообщения (доказательство — счётчик инспектора
  15 → 17 и рост id последнего сообщения) → **ни одного входящего кадра**.

**Вывод.** У 4PDA нет живого событийного канала по :993 ни в легаси-варианте (`ea` — заглушка,
принимающая что угодно), ни в новом протоколе (request/response + idle-таймаут). События сервер
раздаёт ТОЛЬКО через FCM. Практические следствия для ProPDA:
- «мгновенно» = push (Pro-ключ + разовый app-логин); без него — страховочный опрос;
- `isWebSocketConnected()` нельзя считать признаком доставки НИГДЕ в коде;
- поднимать realtime-сокет ради событий смысла нет; он остаётся только как исторический путь.
