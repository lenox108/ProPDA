...
Чтобы не грузить сотни мегабайт со своего интернета, есть
`.github/workflows/icon-variants.yml`: ручной запуск (Actions → Icon variants →
тег релиза) скачивает базовый APK релиза внутри GitHub, патчит, подписывает и
прикрепляет варианты обратно. Нужны 4 секрета с ключом подписи — список в самом
workflow.
